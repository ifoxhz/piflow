 ...
 * Output:
 *   dist-windows/piFlow/…
 *   dist-windows/piFlow-0.1.0-portable.zip
 */
...
const OUT_DIR = path.join(OUT_ROOT, 'piFlow');
const pkg = JSON.parse(readFileSync(path.join(ROOT, 'package.json'), 'utf-8'));
const version = pkg.version || '0.1.0';
const ZIP = path.join(OUT_ROOT, `piFlow-${version}-portable.zip`);
...
copyFileSync(EXE, path.join(OUT_DIR, 'piFlow.exe'));
...
try {
  ...
  zipOut = path.join(OUT_ROOT, `piFlow-${version}-portable-${Date.now()}.zip`);
  console.warn(`[portable] existing zip locked; writing ${zipOut}`);
}
execSync(`tar -a -c -f "${zipOut}" -C "${OUT_ROOT}" piFlow`, {
  stdio: 'inherit',
  shell: true,
});
